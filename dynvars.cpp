...
class announceScopeEnd
{
    public:
        ~announceScopeEnd() { printf("{scope ended}\n"); }
};
...
void test()
{
    ...
    *aFloat = 3.14f;
    ...
    // create another variable for a non-fundamental custom class
    printf("another someClass should be created next and automatically destroyed at scope end:\n");
    vars.GetAuto<someClass>("someClassAnotherInstsance")->secondVal = 333;
    // but don't manually remove it, the "vars" local variable will be destroyed when the scope ends which will cause all variables to be destroyed automatically

    announceScopeEnd scope;
}
